...
export default function EditRentalPage() {
  ...
  useEffect(() => {
    Promise.all([fetchRental(), fetchCustomers(), fetchBungalows()]);
  }, [rentalId]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          bungalowId: data.bungalowId.toString(),
          startDate,
          endDate,
        });
      } else {
        setError("Kiralama bulunamadı");
      }
    } catch (err) {
      setError("Kiralama yükleme hatası");
      console.error(err);
    } finally {
  ...
  const fetchBungalows = async () => {
    try {
      ...
      console.error("Bungalowlar yüklenemedi:", err);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setError(data.error || "Kiralama güncellenirken hata oluştu");
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-violet-100 flex items-center justify-center">
        <div className="text-center">
          <div className="text-lg text-gray-600">Kiralama bilgileri yükleniyor...</div>
        </div>
      </div>
    );
  }
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex justify-between items-center">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900">✏️ Kiralama Güncelle</h1>
          </div>
        </div>
      </header>
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              >
                {loading ? "Güncelleniyor..." : "Kiralama Güncelle"}
              </button>
              <Link
                href="/rentals"
